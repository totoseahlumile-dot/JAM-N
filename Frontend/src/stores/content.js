...
const load = (resource, request, select) => async ({ commit }) => {
  commit("SET_LOADING", { resource, value: true }); commit("SET_ERROR", { resource, value: null });
  try { const value = select(await request()); commit("SET_RESOURCE", { resource, value }); return value; }
...
const actions = {
  ...
  fetchPosts: load("posts", () => apiRequest("/api/posts?limit=50"), (body) => body.posts)
};
const getters = {
