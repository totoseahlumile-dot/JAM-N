...
Run `npm run migrate` after merging this branch. The payment API offers Plus (R49) and Pro (R129) as one-time 30-day terms. Set the `PAYFAST_*` values in `.env` from your own sandbox account, including a public HTTPS `PAYFAST_NOTIFY_URL` ending in `/api/payments/payfast/notify`. A local-only callback cannot receive PayFast notifications. Live payments are intentionally disabled.

Use a Bearer token from `POST /api/auth/login` for `POST /api/payments/checkout` (`{ "planId": "plus" }` or `"pro"`) and `GET /api/payments/subscription`. POST the returned fields as a form to the returned sandbox checkout URL. The return page is informational only; verified PayFast ITNs activate terms. `GET /api/payments/orders/:id` shows an order owned by the authenticated user. Beat checkout is disabled until licenses, rights, and a server-side catalog exist.
